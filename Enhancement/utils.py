...
def load_img_HSV(filepath):
    return cv2.cvtColor(cv2.imread(filepath), cv2.COLOR_BGR2HSV_FULL)
def save_img(filepath, img):
    cv2.imwrite(filepath, cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
def save_img_HSV(filepath, img):
    cv2.imwrite(filepath, cv2.cvtColor(img, cv2.COLOR_HSV2BGR_FULL))
